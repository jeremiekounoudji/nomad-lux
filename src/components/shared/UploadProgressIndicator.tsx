...
const UploadProgressIndicator: React.FC<UploadProgressIndicatorProps> = ({ 
  ...
  return (
    <Card className={`border border-blue-200 bg-blue-50 p-4 ${className}`}>
          ...
          {uploads.map((upload, index) => (
            <div key={`${upload.fileName}-${index}`} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                <span className="max-w-[200px] truncate text-gray-700" title={upload.fileName}>
                  {upload.fileName}
                </span>
                <div className="flex items-center space-x-2">
                  ...
                  {upload.status === 'error' && (
                    <svg className="size-3 text-red-600" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                    </svg>
                  )}
                  {upload.status === 'uploading' && (
                    <div className="size-3">
                      <div className="size-3 animate-spin rounded-full border-b-2 border-blue-600"></div>
                    </div>
                  )}
        ...
        <div className="border-t border-blue-200 pt-2 text-xs text-blue-700">
          {completedUploads === totalUploads ? (
            <span className="flex items-center">
              <svg className="mr-1 size-3 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
              </svg>
