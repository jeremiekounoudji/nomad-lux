...
const Sidebar: React.FC = () => {
  ...
  const navigationItems = [
    ...
    {
      path: ROUTES.LIKED_PROPERTIES,
      label: t('navigation.menu.favorites'),
      icon: Heart,
      requireAuth: true,
    },
    {
      path: ROUTES.CREATE_PROPERTY,
      label: t('navigation.menu.createProperty'),
      icon: Plus,
      requireAuth: true,
    },
    {
      path: ROUTES.MY_LISTINGS,
      label: t('navigation.menu.myListings'),
      icon: Home,
      requireAuth: true,
    },
    {
      path: ROUTES.MY_BOOKINGS,
      label: t('navigation.menu.bookings'),
      icon: Calendar,
      requireAuth: true,
    },
    {
    ...
    },
    {
      ...
      label: t('navigation.menu.notifications'),
      icon: Bell,
      requireAuth: true,
    },
      ...
      <div className="shrink-0 p-4">
        {isAuthenticated && user ? (
          <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-md transition-shadow duration-200 hover:shadow-lg">
            <div className="flex items-center gap-3">
              ...
              <div className="relative">
                <img
                  src={
                    user.avatar_url ||
                    `https://ui-avatars.com/api/?name=${encodeURIComponent(user.display_name)}&background=3B82F6&color=fff`
                  }
                ...
                {user.is_email_verified && (
                  <div className="absolute -bottom-1 -right-1 flex size-4 items-center justify-center rounded-full border-2 border-white bg-green-500 shadow-sm">
                    <span className="text-[8px] font-bold text-white">{t('profile.verified')}</span>
                  </div>
                )}
              </div>
            ...
            <NavLink
              to="/profile"
              className="mt-3 w-full rounded-lg bg-blue-600 px-3 py-2 text-center text-xs font-medium text-white transition-colors hover:bg-blue-700"
            >
            ...
            <div className="mt-3 flex justify-between gap-2">
              <div className="flex-1 rounded-lg bg-gray-50 p-2">
                <div className="text-xs font-bold text-gray-900">{user.total_properties || 0}</div>
                <div className="text-[10px] text-gray-500">
                  {t('navigation.profile.properties')}
                </div>
              </div>
              <div className="flex-1 rounded-lg bg-gray-50 p-2">
                <div className="text-xs font-bold text-gray-900">{user.total_bookings || 0}</div>
                <div className="text-[10px] text-gray-500">{t('navigation.profile.bookings')}</div>
              </div>
            </div>
          </div>
        ) : (
          <div className="rounded-xl border border-gray-100 bg-white p-4 shadow-md transition-shadow duration-200 hover:shadow-lg">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-bold text-gray-900">
                  {t('navigation.profile.guestUser')}
                </h3>
            ...
            <div className="mt-3 flex gap-2">
              <NavLink
                to={ROUTES.LOGIN}
                className="flex-1 rounded-lg bg-primary-600 p-2 text-center text-xs font-medium text-white transition-colors hover:bg-primary-700"
              >
                {t('auth.login.signIn')}
              </NavLink>
              <NavLink
                to={ROUTES.REGISTER}
                className="flex-1 rounded-lg bg-gray-200 p-2 text-center text-xs font-medium text-gray-700 transition-colors hover:bg-gray-300"
              >
                ...
                (!item.requireAuth || isAuthenticated) && (
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        <item.icon
                          className={`size-6 ${isActive ? 'text-primary-600' : 'text-gray-500'}`}
                        />
              ...
              {secondaryItems.map((item) => (
                ...
                >
                  {({ isActive }) => (
                    <>
                      <item.icon
                        className={`size-5 ${isActive ? 'text-primary-600' : 'text-gray-500'}`}
                      />
