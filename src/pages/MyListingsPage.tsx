...
import {
  convertDatabasePropertyToProperty,
  getStatusColor,
...
const MyListingsPage: React.FC<MyListingsPageProps> = ({ onPageChange }) => {
  ...
  return (
    <>
        ...
        <div className="w-full">
          ...
          >
            <Tabs
              ...
              classNames={{
                tabList:
                  'gap-2 sm:gap-4 md:gap-6 w-max relative rounded-none p-0 border-b border-divider',
                cursor: 'w-full bg-primary-500',
                tab: 'max-w-fit px-2 sm:px-3 md:px-4 h-12 flex-shrink-0',
                tabContent:
                  'group-data-[selected=true]:text-primary-600 text-xs sm:text-sm md:text-base whitespace-nowrap',
              }}
            >
              <Tab key="all" title={`${t('property.listings.tabs.all')} (${statusCounts.all})`} />
              <Tab
                key="approved"
                title={`${t('property.listings.tabs.approved')} (${statusCounts.approved})`}
              />
              <Tab
                key="pending"
                title={`${t('property.listings.tabs.pending')} (${statusCounts.pending})`}
              />
              <Tab
                key="paused"
                title={`${t('property.listings.tabs.paused')} (${statusCounts.paused})`}
              />
              <Tab
                key="rejected"
                title={`${t('property.listings.tabs.rejected')} (${statusCounts.rejected})`}
              />
            </Tabs>
          </div>
        </div>
      ...
      {/* Loading State */}
      {isLoading && <PropertyListingSkeleton className="col-span-1 md:col-span-2 lg:col-span-4" />}
        ...
        <div className="col-span-1 py-12 text-center md:col-span-2 lg:col-span-4">
          <div className="rounded-lg border border-red-200 bg-red-50 p-6">
            <h3 className="mb-2 text-lg font-medium text-red-800">
              {t('property.listings.errors.loadingListings')}
            </h3>
                          ...
                          <div>
                            ...
                            <div className="mt-2 flex items-center gap-1">
                              ...
                              <span className="text-sm text-gray-500">
                                ({formatPrice(listing.price_per_night, listing.currency || 'USD')}
                                /night)
                              </span>
                            </div>
                          </div>
                            ...
                            <div className="grid grid-cols-3 gap-2 text-center">
                              <div>
                                ...
                                <p className="text-xs text-gray-500">
                                  {t('property.listings.stats.views')}
                                </p>
                              </div>
                              <div>
                                <p className="text-sm font-bold text-green-600">{stats.bookings}</p>
                                <p className="text-xs text-gray-500">
                                  {t('property.listings.stats.bookings')}
                                </p>
                              </div>
                              <div>
                                <p className="text-sm font-bold text-primary-600">
                                  {formatPrice(stats.revenue, listing.currency || 'USD')}
                                </p>
                                <p className="text-xs text-gray-500">
                                  {t('property.listings.stats.revenue')}
                                </p>
                              </div>
                            </div>
                          )}
            ...
            <div className="col-span-1 mt-6 flex items-center justify-between border-t border-gray-100 pt-4 md:col-span-2 lg:col-span-4">
              ...
              <div className="flex items-center space-x-4">
                <span className="text-sm text-gray-700">
                  {t('property.listings.pagination.pageOf', {
      ...
      {!isLoading && !error && filteredListings.length === 0 && (
        <div className="col-span-1 py-12 text-center md:col-span-2 lg:col-span-4">
          ...
          <h3 className="mb-2 text-lg font-medium text-gray-900">
            {statusFilter === 'all'
              ? t('property.listings.empty.noListingsDescription')
              : t('property.listings.empty.noListings', { status: statusFilter })}
          </h3>
            ...
            <>
              <ModalHeader className="flex flex-col gap-1">
                ...
                <p className="text-sm text-gray-600">
                  {t('property.listings.modals.deleteConfirmation')}
                </p>
              </ModalHeader>
              <ModalBody>
                {propertyToDelete && (
